...
void USART_Init(USART_TypeDef* USARTx, USART_InitTypeDef* USART_InitStruct);//串口初始化，波特率，数据字长，奇偶校验，硬件流控，收发使能
...
uint16_t USART_ReceiveData(USART_TypeDef* USARTx);                          //接收数据，从DR读取接收到的数据
...
ITStatus USART_GetITStatus(USART_TypeDef* USARTx, uint16_t USART_IT);      //获取中断状态标志位
