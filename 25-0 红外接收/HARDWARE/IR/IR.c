...
{  
	...
	GPIO_SetBits(GPIOA,GPIO_Pin_1);						 //PA0 下拉
	...
	TIM_ICInitStructure.TIM_Channel = TIM_Channel_2; //CC1S=01 	选择输入端 IC1映射到TI1上
  	...
  	TIM_ICInitStructure.TIM_ICFilter = 0x03;//IC1F=0000 配置输入滤波器 不滤波
	...
	NVIC_InitStruct.NVIC_IRQChannelSubPriority=3;  //设置外部中断响应优先级
